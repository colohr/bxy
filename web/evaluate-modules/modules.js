...
})(this, async function ModulesExporter(environment, element, Project, Cookie, Logger, Keyboard){
	let load = []
...
async function get_element(script = null){
	...
	async function load_meta(element,base=null){
		try{
			...
			else  base = await window.modules.http(element.url).json(false, {}).catch(on_invalid_package)
		}
	...
	function project_package_module_resource(){
		...
		async function load_package_json(package_module_resource, ...package_annotation){
			...
			catch(error){ console.error(package_module_resource.error = error) }
