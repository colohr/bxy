(function define(...x){const _=this.modules?this.modules:module,__=(m,...a)=>_.define?_.define('function.scope.environment',{value:m(...a)}):_.exports=m(...a);return _.has&&_.has('function.scope.environment')?_.get('function.scope.environment'):(([m],y,...z)=>__(m,...(y.concat(z))))(x.splice(0,1),(x=x.map(i=>Array.isArray(i)?i.map(f=>f()):i).reduce((l,i)=>(Array.isArray(i)?l[0].push(...i):l.push(i),l),[[]]))[0],...x.slice(1, x.length));})
(function definition(...x) {
	...
	function get_scope_exporter(field = null){
		switch(scope_environment){
			case 'node_module':
				return (...inputs)=>(defines(inputs) ? module.exports[field] = inputs[0]:module.exports = inputs[0])
			case 'web_module':
				return (...inputs)=>(defines(inputs) ? window.modules.set(field, inputs[0]):inputs[0])
			case 'web_worker':
				return (...inputs)=>(defines(inputs) ? this[field] = inputs[0]:inputs[0])
			case 'window':
				return (...inputs)=>(defines(inputs) ? window[field] = inputs[0]:inputs[0])
		}
