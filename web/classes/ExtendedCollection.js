...
async function Locations(){
	...
	function create_collection_locations(collection, base = null){
		...
		if('locations' in base){
			for(const field of base.locations){
				...
				else if(is.array(value)) value = URL.join(...value, base.folder)
	...
	function get_collection_url(collection){
		return function create_collection_url(notation, ...locator){
			if(is.text(notation.indexOf('http') === 0)) notation = new URL(notation)
			if(notation instanceof URL) locator = locator.length ? URL.join(notation, ...locator):notation
			else if(collection.has(`locations.${notation}`)) locator = URL.join(collection.get(`locations.${notation}`), ...locator)
			else locator = URL.join(collection.base.url, notation,...locator)
			if(locator.file !== locator.basename) locator.field = locator.basename
