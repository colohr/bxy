...
modules.import = (function(){
	...
	const module_import = {assets: import_scripts,class: import_asset,function: load_function,json: import_asset,meta: import_asset,mixin: import_asset,module: import_asset, package:import_package, script: import_script}
	//exports
	...
	//scope actions
	function get_locator(locator){
		if(typeof locator === 'string') locator = URL.is(locator) ? new URL(locator):new URL(locator, window_locator)
		return locator
	}
...
//exports
worker.once('environment', async function define_environment(event){
	modules.set('directory.package', worker.detail(event).directory.package)
	modules.set('directory.base', set_locators(worker.detail(event).directory.base))
	modules.set('project', set_locators(worker.detail(event).project))
	return (event=null, typeof on_environment === 'function' ? await on_environment():null, worker.send('environment'))
	//scope actions
	function set_locators(data, locators={}){
		for(const entry of Object.entries(data)){
			modules.dot.set(locators, entry[0], URL.is(entry[1]) ? new URL(entry[1]):entry[1])
		}
